...
export class AuthService {
  ...
  getRequestToken() {
    return this._http.get(`authentication/token/new`).do((response: any) => {
      const token = response.request_token;
      const redirectTo = `http://${location.host}/login`;
      location.href = `${
        environment.THEMOVIE_DB_BASE_URL
      }/authenticate/${token}?redirect_to=${redirectTo}`;
  ...
  getSession(request_token: string) {
    ...
    return this._http
      .get(`authentication/session/new`, { params })
